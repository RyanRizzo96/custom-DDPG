...
class NormalActionNoise(ActionNoise):
    ...
    :param mean: (float) mean value of the noise
    :param sigma: (float) the scale of the noise (standard deviation here)
    """
    ...
    def __call__(self):
        """
        Create normal distribution according to desired mean and standard deviation
        :return: norma distribution
        """
        return np.random.normal(self._mean, self._sigma)

    def __repr(self):
        return 'NormalActionNoise(mu={}, sigma={})'.format(self._mean, self._sigma)


class OrnsteinUhlenbeckActionNoise(ActionNoise):
    """
    Ornstein Uhlenbeck action noise, designed tto approximate brownian motion with friction
    ...
    def __call__(self):
        noise = self.noise_prev + self._theta * (self._mean - self.noise_prev) * self._dt + \
                self._sigma * np.sqrt(self._dt) * np.random.normal(size=self._mean.shape)
